...
final class CondominioTable extends PowerGridComponent
{
    // use WithExport;
    use Actions;
    use HelperActions;
    ...
    {
        $tenants = Tenant::query()->with('users:id');

        if(!auth()->user()->isAdmin()) {
            $tenants->whereHas('users', fn($query) => $query->where('user_id', auth()->id()));
        }
    ...
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ...
            ->addColumn('id_tipo_condominio', function (Tenant $condominio) {
                return $condominio->getTipoCondominio()['name'];
            })
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Tipo', 'id_tipo_condominio')
            ->sortable(),
    ...
    public function filters(): array
    {
        return [
            ...
            /* Mais lento */
            Filter::select('id_tipo_condominio')
            ->dataSource(Tenant::$tipos_condominios)
            ->optionValue('id')
            ->optionLabel('name'),

            /* Mais rapido */
            // Filter::boolean('type')
            // ->label('Condominio', 'Usuário'),
    ...
    public function delete($rowId, $params=null): void
    {
        if($params == null) {
            $this->dialog()->confirm([
                ...
                'description' => 'Deletar esta condominio?<br/>Uma vez deletada, todos os dados seram apagados.',
        ...
        try {
            ...
            $this->notification([
                'title'       => 'Condominio deletada!',
                'description' => 'Condominio foi deletada com sucesso',
                'icon'        => 'success'
            ]);
            ...
            $this->notification([
                ...
                'description' => 'Não foi possivel deletar a Condominio.',
                'icon'        => 'error'
            ]);
        }
    }
    ...
    public function actions(Tenant $row): array
    {
        return [
            ...
            Button::add('edit')
                ...
                ->class('pg-btn-white dark:ring-pg-primary-600 dark:border-pg-primary-600 dark:hover:bg-pg-primary-700 dark:ring-offset-pg-primary-800 dark:text-pg-primary-300 dark:bg-pg-primary-700')
                ->id()
                ->can(auth()->user()->isAdmin() || $row->users->firstWhere('id', auth()->id()))

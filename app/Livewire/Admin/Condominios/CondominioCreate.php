...
class CondominioCreate extends Component
{
    use Actions;
    use HelperActions;
    ...
    public function save($params=null)
    {
        ...
        try {
            ...
            $condominio = Tenant::create($this->form->all());

            $condominio->users()->attach(auth()->user());

            $horarios = $this->form->getHours();
            if($horarios) $condominio->horarios()->createMany($horarios);
            ...
            $this->dialog()->confirm([
                'title'       => 'Deseja acessar o Painel da Condominio?',
                // 'description' => 'Deseja alterar o preço?',
                'icon'        => 'question',
                'accept'      => [
                    'label'  => 'Sim',
                    'method' => 'saved',
                    'params' => [$condominio->id],
                ],
                'onClose' => [
                    'method' => 'saved',
                ]
    ...
    public function saved($params=null)
    {
        if($params) {
            ...
            $this->redirect(route('admin.condominios.index'), true);
        }
    }

    public function render()
    {
        return view('livewire.admin.condominios.condominio-create');
    }
}

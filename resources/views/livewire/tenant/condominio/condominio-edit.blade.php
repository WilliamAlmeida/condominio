            ...
            <div class="bg-white dark:bg-gray-700 overflow-hidden shadow-sm sm:rounded-lg pb-2">
                <div class="p-2 md:p-2 text-gray-900 dark:text-gray-100">
                    ...
                    <div x-data="{ activeTab: 'dados' }">
                        @if($condominio || !$readMode && !$condominio)
                        ...
                        <div id="default-tab-content">
                            ...
                            <div x-show="activeTab === 'dados'" class="p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="dados" role="tabpanel" aria-labelledby="dados-tab">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    @if($readMode)
                                        <x-input label="Tipo de Condominio" value="{{ $condominio ? ($condominio->getTipoCondominio()['name'] ?? null) : null }}" disabled="true" />
                                    @endif
                                    <x-select label="Tipo de Condominio" placeholder="Selecione um Tipo" :options="$array_tipos_condominios" option-label="name" option-value="id" wire:model.defer="form.id_tipo_condominio" :class="$readMode ? 'hidden' : null" />
                                    ...
                                    <div class="col-span-1 sm:col-span-2">
                                        <div class="grid grid-cols-3 sm:grid-cols-3 gap-4">
                                            ...
                                            @if($readMode)
                                                <x-input label="Estado" value="{{ $condominio->estado->uf ?? null }}" disabled="true" />
                                            @endif
                                            <x-select label="Estado" placeholder="Selecione um Estado" :options="$array_estados" option-label="uf" option-value="id" wire:model.defer="form.estado_id" :class="$readMode ? 'hidden' : null" />
                            ...
                            <div class="flex justify-center gap-x-3">
                                ...
                                @else
                                    <div class="mt-3">
                                        <x-button primary :label="$condominio ? 'Editar' : 'Registrar'" class="w-24" wire:click="$set('readMode', false)" />
                                    </div>
                                @endif
                            </div>

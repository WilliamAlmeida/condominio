...
class RolesAndPermissionsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $arrayOfPermissionsNames = [
            'countries', 'states', 'cities', 'users', 'roles', 'permissions', 'tenants',
            'tenant.users', 'tenant.tenants'
        ];
        ...
        $usuario = User::where('type', User::ADMIN)->first();
        if($usuario) {
            $usuario->syncRoles(Role::findByName('super admin'));
        }

        $usuario = User::where('type', User::CONDOMINIO)->first();
        if($usuario) {
            $usuario->syncRoles(Role::findByName('admin'));
        }

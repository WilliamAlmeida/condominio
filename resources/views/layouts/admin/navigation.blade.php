<aside id="default-sidebar" class="fixed sm:relative top-0 left-0 z-40 w-64 md:w-72 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidenav">
    <div class="overflow-y-auto py-5 px-3 h-full bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <ul class="space-y-2">
            <li>
                <x-layouts.sidebar.link :route="route('admin.dashboard')" label="Dashboard" :active="request()->routeIs('admin.dashboard')">
                    <x-slot name="icon">
            ...
            @can('users.viewAny')
            <li>
                <x-layouts.sidebar.link :route="route('admin.usuarios.index')" label="Usuários" :active="request()->routeIs('admin.usuarios.index')">
                    <x-slot name="icon">
            ...
            @endcan
            <li>
                <x-layouts.sidebar.link :route="route('admin.condominios.index')" label="Condomínios" :active="request()->routeIs('admin.condominios.index')">
                    <x-slot name="icon">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M4 4a1 1 0 0 1 1-1h14a1 1 0 1 1 0 2v14a1 1 0 1 1 0 2H5a1 1 0 1 1 0-2V5a1 1 0 0 1-1-1Zm5 2a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1h1a1 1 0 0 0 1-1V7a1 1 0 0 0-1-1H9Zm5 0a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1h1a1 1 0 0 0 1-1V7a1 1 0 0 0-1-1h-1Zm-5 4a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1h1a1 1 0 0 0 1-1v-1a1 1 0 0 0-1-1H9Zm5 0a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1h1a1 1 0 0 0 1-1v-1a1 1 0 0 0-1-1h-1Zm-3 4a2 2 0 0 0-2 2v3h2v-3h2v3h2v-3a2 2 0 0 0-2-2h-2Z" clip-rule="evenodd"/></svg>
                    </x-slot>
                </x-layouts.sidebar.link>
            </li>
            @canany('permissions.viewAny', 'roles.viewAny')
            <li>
                <x-layouts.sidebar.dropdown label="Funções e Permissões" :open="request()->routeIs('admin.funcoes.index') || request()->routeIs('admin.permissoes.index')">
                    <x-slot name="icon">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M17 10v1.126c.367.095.714.24 1.032.428l.796-.797 1.415 1.415-.797.796c.188.318.333.665.428 1.032H21v2h-1.126c-.095.367-.24.714-.428 1.032l.797.796-1.415 1.415-.796-.797a3.979 3.979 0 0 1-1.032.428V20h-2v-1.126a3.977 3.977 0 0 1-1.032-.428l-.796.797-1.415-1.415.797-.796A3.975 3.975 0 0 1 12.126 16H11v-2h1.126c.095-.367.24-.714.428-1.032l-.797-.796 1.415-1.415.796.797A3.977 3.977 0 0 1 15 11.126V10h2Zm.406 3.578.016.016c.354.358.574.85.578 1.392v.028a2 2 0 0 1-3.409 1.406l-.01-.012a2 2 0 0 1 2.826-2.83ZM5 8a4 4 0 1 1 7.938.703 7.029 7.029 0 0 0-3.235 3.235A4 4 0 0 1 5 8Zm4.29 5H7a4 4 0 0 0-4 4v1a2 2 0 0 0 2 2h6.101A6.979 6.979 0 0 1 9 15c0-.695.101-1.366.29-2Z" clip-rule="evenodd"/></svg>
                    </x-slot>

                    <x-layouts.sidebar.sublink :route="route('admin.funcoes.index')" label="Funções" :active="request()->routeIs('admin.funcoes.index')" />
                    <x-layouts.sidebar.sublink :route="route('admin.permissoes.index')" label="Permissões" :active="request()->routeIs('admin.permissoes.index')" />

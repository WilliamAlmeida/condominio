...
final class UsuarioTable extends PowerGridComponent
{
    ...
    {
        return User::query()->with('foto:id,url');
    }
    ...
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ->addColumn('id')
            ->addColumn('foto', fn($row) => $this->fieldImage($row, url: $row->foto ? asset('storage/'.$row->foto->url) : null))
            ->addColumn('name')
    ...
    public function login($rowId): void
    {
        auth()->login(User::findOrFail($rowId));
